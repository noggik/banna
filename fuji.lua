...
-- ฟังก์ชันอัปเดต Dropdown (แก้ไขใหม่)
local function refreshBankDropdown()
    local modelNames = getAllBankModels()
    
    if #modelNames > 0 then
        -- เพิ่ม "Select a model" เป็นตัวเลือกแรก
        local dropdownValues = {"Select a model"}
        for _, modelName in ipairs(modelNames) do
            table.insert(dropdownValues, modelName)
        end
        
        teleportDropdown:SetValues(dropdownValues)
        teleportDropdown:SetValue("Select a model") -- ตั้งให้แสดง "Select a model" แทน
        print("🔄 Refreshed Bank models: " .. #modelNames .. " models found")
        
        -- แสดงรายชื่อโมเดลที่พบ
        local modelList = table.concat(modelNames, ", ")
        print("📋 Models: " .. modelList)
    else
        teleportDropdown:SetValues({"No models found"})
        teleportDropdown:SetValue("No models found")
        print("⚠️ No models found in workspace.Bank")
    end
end

-- Dropdown สำหรับเลือกโมเดล (แก้ไขใหม่)
teleportDropdown = TeleportSection:AddDropdown("BankTeleport", {
    Title = "TP BANK",
    Description = "เลือกโมเดลใน workspace.Bank เพื่อ Teleport ไป",
    Values = {"Click Refresh first"},
    Multi = false,
    Default = "Click Refresh first",
    Callback = function(value)
        if value ~= "Click Refresh first" and value ~= "No models found" and value ~= "Select a model" then
            teleportToModel(value)
        elseif value == "Click Refresh first" then
            print("💡 กรุณากดปุ่ม Refresh ก่อนใช้งาน")
        elseif value == "Select a model" then
            print("👆 กรุณาเลือกโมเดลที่ต้องการ Teleport ไป")
        end
    end
})
